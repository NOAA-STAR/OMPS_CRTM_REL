...
MODULE ODPS_Predictor
  ...
  INTEGER, PARAMETER  :: N_COMPONENTS_G(N_G)     = (/8,   5, 2  , 2/)
  INTEGER, PARAMETER  :: N_ABSORBERS_G(N_G)      = (/ 6,  3,  1,  1/)
  INTEGER, PARAMETER  :: MAX_N_PREDICTORS_G(N_G) = (/18, 15, 14, 14/)
...
!                    13, &  ! ozone
  ...
  INTEGER, PARAMETER :: N_PREDICTORS_G4(2) = (/ &
                     7, &  ! dry gas
                    11 /)  ! ozone
  ...
  ! UV sensor Component indexes (sequence index in an array)
  INTEGER, PARAMETER :: COMP_DRY_UV = 1
  INTEGER, PARAMETER :: COMP_OZO_UV = 2
  ...
  INTEGER, PARAMETER :: COMPONENT_ID_MAP_G4(2) = (/ &
                                   13, &
                                   14 /)
  ...
  INTEGER,  PARAMETER :: ABSORBER_ID_MAP_G4(1) = (/ &
                                           O3_ID /)
  ! Literal constants
...
!------------------------------------------------------------------------------
  ...
  SUBROUTINE ODPS_Compute_Predictor( &
    ...
    SELECT CASE( Group_ID )
      ...
      CASE( GROUP_4 )
         CALL ODPS_Compute_Predictor_UV()
    END SELECT
...
!        Predictor%X(k, 13, COMP_OZO_IR)  = SECANG(k)*GAzp(k,ABS_H2O_IR)
    ...
    SUBROUTINE ODPS_Compute_Predictor_UV()
      ...
      Layer_Loop : DO k = 1, n_Layers
        ...
        !  Abosrber amount scalled by the reference
        !-------------------------------------------
        O3  = Absorber(k,ABS_O3_UV) /Ref_absorber(k,ABS_O3_UV)
        ...
        ! set number of predictors
        Predictor%n_CP = N_PREDICTORS_G4
       ...
       !#-------------------------------------------------------------------#
        ...
        !   Fixed (Dry) predictors
        !  ----------------------
        Predictor%X(k, 1, COMP_DRY_UV)  = SECANG(k)
        Predictor%X(k, 2, COMP_DRY_UV)  = SECANG(k) * T
        Predictor%X(k, 3, COMP_DRY_UV)  = SECANG(k) * T2
        Predictor%X(k, 4, COMP_DRY_UV)  = T
        Predictor%X(k, 5, COMP_DRY_UV)  = SECANG(k) * SECANG(k)
        Predictor%X(k, 6, COMP_DRY_UV)  = T2
        Predictor%X(k, 7, COMP_DRY_UV)  = Tz(k)

        !  -----------------------
        !  Ozone predictors  RTTOV ver 1
        !  -----------------------
        Predictor%X(k, 1, COMP_OZO_UV)  = O3_A                                        ! SECANG(k)*O3
        Predictor%X(k, 2, COMP_OZO_UV)  = O3_A*DT                                     ! SECANG(k)*O3*dT
        Predictor%X(k, 3, COMP_OZO_UV)  = O3_A*O3            *GAzp(k,ABS_O3_UV)       ! SECANG(k)*O3**2*O3w 
        Predictor%X(k, 4, COMP_OZO_UV)  = O3_A*O3_A                                   ! (SECANG(k)*O3)**2
        Predictor%X(k, 5, COMP_OZO_UV)  = O3_A               *GAzp(k,ABS_O3_UV)       ! SECANG(k)*O3*O3w 
        Predictor%X(k, 6, COMP_OZO_UV)  = O3_A*SQRT(SECANG(k)*GAzp(k,ABS_O3_UV))      ! SECANG(k)*O3*sqrt(SECANG(k)*O3w)
        Predictor%X(k, 7, COMP_OZO_UV)  = O3_R*DT   !T*T*T                            ! SQRT(SECANG(k)*O3)*dT    
        Predictor%X(k, 8, COMP_OZO_UV)  = O3_R                                        ! SQRT(SECANG(k)*O3)  
        Predictor%X(k, 9, COMP_OZO_UV)  = O3_R*O3   /GAzp(k,ABS_O3_UV)                ! SQRT(SECANG(k)*O3)*O3/O3w
        Predictor%X(k,10, COMP_OZO_UV)  = SECANG(k) *GAzp(k,ABS_O3_UV)                ! SECANG(k)*O3w
        Predictor%X(k,11, COMP_OZO_UV)  = (SECANG(k)*GAzp(k,ABS_O3_UV))**2            ! (SECANG(k)*O3w)**2

      END DO Layer_Loop

    END SUBROUTINE ODPS_Compute_Predictor_UV
...
!!    TYPE(PAFV_type), POINTER  :: PAFV => NULL()
    ...
    SELECT CASE( Group_ID )
      ...
      CASE( GROUP_4 )
         CALL ODPS_Compute_Predictor_UV_TL()
    END SELECT
...
!        Predictor_TL%X(k, 13, COMP_OZO_IR)  = SECANG(k)*GAzp_TL(k,ABS_H2O_IR)
   ...
   SUBROUTINE ODPS_Compute_Predictor_UV_TL()
      ...
      Layer_Loop : DO k = 1, n_Layers
        ...
        O3_TL  = Absorber_TL(k,ABS_O3_UV)/Ref_absorber(k,ABS_O3_UV)
        ...
        Predictor_TL%n_CP = N_PREDICTORS_G4
       ...
       !#-------------------------------------------------------------------#
        ...
        !   Fixed (Dry) predictors
        !  ----------------------
        Predictor_TL%X(k, 1, COMP_DRY_UV)  = ZERO
        Predictor_TL%X(k, 2, COMP_DRY_UV)  = SECANG(k) * T_TL
        Predictor_TL%X(k, 3, COMP_DRY_UV)  = SECANG(k) * T2_TL
        Predictor_TL%X(k, 4, COMP_DRY_UV)  = T_TL
        Predictor_TL%X(k, 5, COMP_DRY_UV)  = ZERO
        Predictor_TL%X(k, 6, COMP_DRY_UV)  = T2_TL
        Predictor_TL%X(k, 7, COMP_DRY_UV)  = Tz_TL(k)

        !  -----------------------
        !  Ozone predictors
        !  -----------------------
        Predictor_TL%X(k, 1, COMP_OZO_UV)  = O3_A_TL
        Predictor_TL%X(k, 2, COMP_OZO_UV)  = O3_A_TL*DT + O3_A*DT_TL
        Predictor_TL%X(k, 3, COMP_OZO_UV)  = O3_A_TL*O3*PAFV%GAzp(k,ABS_O3_IR) + O3_A*O3_TL*PAFV%GAzp(k,ABS_O3_IR) &
                                             + O3_A*O3*GAzp_TL(k,ABS_O3_IR)
        Predictor_TL%X(k, 4, COMP_OZO_UV)  = TWO*O3_A*O3_A_TL
        Predictor_TL%X(k, 5, COMP_OZO_UV)  = O3_A_TL*PAFV%GAzp(k,ABS_O3_IR) + O3_A*GAzp_TL(k,ABS_O3_IR)
        Predictor_TL%X(k, 6, COMP_OZO_UV)  = O3_A_TL*SQRT(SECANG(k)*PAFV%GAzp(k,ABS_O3_IR)) + &
                                             POINT_5*O3_A*SQRT(SECANG(k)/PAFV%GAzp(k,ABS_O3_IR))* &
                                             GAzp_TL(k,ABS_O3_IR)
        Predictor_TL%X(k, 7, COMP_OZO_UV)  = O3_R_TL*DT + O3_R*DT_TL  !T*T*T
        Predictor_TL%X(k, 8, COMP_OZO_UV)  = O3_R_TL
        Predictor_TL%X(k, 9, COMP_OZO_UV)  = O3_R_TL*O3/PAFV%GAzp(k,ABS_O3_IR) + O3_R*O3_TL/PAFV%GAzp(k,ABS_O3_IR) &
                                             - O3_R*O3*GAzp_TL(k,ABS_O3_IR)/PAFV%GAzp(k,ABS_O3_IR)**2
        Predictor_TL%X(k,10, COMP_OZO_UV)  = SECANG(k)*GAzp_TL(k,ABS_O3_IR)
        Predictor_TL%X(k,11, COMP_OZO_UV)  = TWO*SECANG(k)**2 * PAFV%GAzp(k,ABS_O3_IR)*GAzp_TL(k,ABS_O3_IR)
      END DO Layer_Loop

    END SUBROUTINE ODPS_Compute_Predictor_UV_TL
...
!!    TYPE(PAFV_type), POINTER  :: PAFV => NULL()
    ...
    SELECT CASE( Group_ID )
      ...
      CASE( GROUP_4 )
         CALL ODPS_Compute_Predictor_UV_AD()
    END SELECT
...
!                  + Predictor_AD%X(k, 13, COMP_OZO_IR)*SECANG(k)
    ...
    SUBROUTINE ODPS_Compute_Predictor_UV_AD()
      ...
      Layer_Loop : DO k = n_Layers, 1, -1
       ...
       !#-------------------------------------------------------------------#
        ...
        !  ----------------------
        T_AD     = T_AD                                            &
                   + Predictor_AD%X(k, 2, COMP_DRY_UV) * SECANG(k) &
                   + Predictor_AD%X(k, 4, COMP_DRY_UV)
        T2_AD    = T2_AD                                           &
                   + Predictor_AD%X(k, 3, COMP_DRY_UV) * SECANG(k) &
                   + Predictor_AD%X(k, 6, COMP_DRY_UV)
        Tz_AD(k) = Tz_AD(k) + Predictor_AD%X(k, 7, COMP_DRY_UV)

        Predictor_AD%X(k, 1, COMP_DRY_UV)  = ZERO
        Predictor_AD%X(k, 2, COMP_DRY_UV)  = ZERO
        Predictor_AD%X(k, 3, COMP_DRY_UV)  = ZERO
        Predictor_AD%X(k, 4, COMP_DRY_UV)  = ZERO
        Predictor_AD%X(k, 5, COMP_DRY_UV)  = ZERO
        Predictor_AD%X(k, 6, COMP_DRY_UV)  = ZERO
        Predictor_AD%X(k, 7, COMP_DRY_UV)  = ZERO
        ...
        O3_A_AD = O3_A_AD                                                               &
                  + Predictor_AD%X(k, 1, COMP_OZO_UV)                                   &
                  + Predictor_AD%X(k, 2, COMP_OZO_UV)*DT                                &
                  + Predictor_AD%X(k, 3, COMP_OZO_UV)*O3*PAFV%GAzp(k,ABS_O3_UV)         &
                  + Predictor_AD%X(k, 4, COMP_OZO_UV)*TWO*O3_A                          &
                  + Predictor_AD%X(k, 5, COMP_OZO_UV)*PAFV%GAzp(k,ABS_O3_UV)            &
                  + Predictor_AD%X(k, 6, COMP_OZO_UV)*SQRT(SECANG(k)*PAFV%GAzp(k,ABS_O3_UV))

        DT_AD   = DT_AD                                                                 &
                  + Predictor_AD%X(k, 2, COMP_OZO_UV)*O3_A                              &
                  + Predictor_AD%X(k, 7, COMP_OZO_UV)*O3_R

        O3_AD   = O3_AD                                                                 &
                  + Predictor_AD%X(k, 3, COMP_OZO_UV)*O3_A*PAFV%GAzp(k,ABS_O3_UV)       &
                  + Predictor_AD%X(k, 9, COMP_OZO_UV)*O3_R/PAFV%GAzp(k,ABS_O3_UV)

        GAzp_AD(k,ABS_O3_UV) = GAzp_AD(k,ABS_O3_UV)                                     &
                  + Predictor_AD%X(k, 3, COMP_OZO_UV)*O3_A*O3                           &
                  + Predictor_AD%X(k, 5, COMP_OZO_UV)*O3_A                              &
                  + Predictor_AD%X(k, 6, COMP_OZO_UV)*POINT_5*O3_A*SQRT(SECANG(k)/PAFV%GAzp(k,ABS_O3_UV)) &
                  - Predictor_AD%X(k, 9, COMP_OZO_UV)*O3_R*O3/PAFV%GAzp(k,ABS_O3_UV)**2 &
                  + Predictor_AD%X(k,10, COMP_OZO_UV)*SECANG(k)                         &
                  + Predictor_AD%X(k,11, COMP_OZO_UV)*TWO*SECANG(k)**2*PAFV%GAzp(k,ABS_O3_UV)

        O3_R_AD = O3_R_AD                                                               &
                  + Predictor_AD%X(k, 7, COMP_OZO_UV)*DT                                &
                  + Predictor_AD%X(k, 8, COMP_OZO_UV)                                   &
                  + Predictor_AD%X(k, 9, COMP_OZO_UV)*O3/PAFV%GAzp(k,ABS_O3_UV)


        Predictor_AD%X(k, 1, COMP_OZO_UV)  = ZERO
        Predictor_AD%X(k, 2, COMP_OZO_UV)  = ZERO
        Predictor_AD%X(k, 3, COMP_OZO_UV)  = ZERO
        Predictor_AD%X(k, 4, COMP_OZO_UV)  = ZERO
        Predictor_AD%X(k, 5, COMP_OZO_UV)  = ZERO
        Predictor_AD%X(k, 6, COMP_OZO_UV)  = ZERO
        Predictor_AD%X(k, 7, COMP_OZO_UV)  = ZERO
        Predictor_AD%X(k, 8, COMP_OZO_UV)  = ZERO
        Predictor_AD%X(k, 9, COMP_OZO_UV)  = ZERO
        Predictor_AD%X(k,10, COMP_OZO_UV)  = ZERO
        Predictor_AD%X(k,11, COMP_OZO_UV)  = ZERO
        ...
        !  Abosrber amount scalled by the reference
        !-------------------------------------------
        Absorber_AD(k,ABS_O3_UV)  = Absorber_AD(k,ABS_O3_UV)             &
                                  + O3_AD / Ref_absorber(k,ABS_O3_UV)
        O3_AD  = ZERO
    ...
    END SUBROUTINE ODPS_Compute_Predictor_UV_AD
...
!!    TYPE(PAFV_type), POINTER  :: PAFV => NULL()
  ...
  PURE FUNCTION ODPS_Get_Component_ID(Component_Index, Group_Index) RESULT( Component_ID )
    ...
    SELECT CASE( Group_Index )
      ...
      CASE( GROUP_4 )
         Component_ID = COMPONENT_ID_MAP_G4(Component_Index)
    END SELECT
  ...
  PURE FUNCTION ODPS_Get_Absorber_ID(Absorber_Index, Group_Index) RESULT(  Absorber_ID )
    ...
    SELECT CASE( Group_Index )
      ...
      CASE( GROUP_4 )
          Absorber_ID = ABSORBER_ID_MAP_G4(Absorber_Index)
    END SELECT
  ...
  PURE FUNCTION ODPS_Get_Ozone_Component_ID(Group_Index) RESULT( Ozone_Component_ID )
    ...
    IF( any(Group_Index == [GROUP_1, GROUP_2, GROUP_4]) ) THEN
